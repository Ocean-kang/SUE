...
class SpectralNet:
    ...
    def fit(self, X: torch.Tensor, y: torch.Tensor = None):
        ...
        L = self._build_laplacian(X)
        Lambda = self.embeddings_.T @ L @ self.embeddings_
        Q, S, _ = np.linalg.svd(Lambda)
        indices = np.argsort(S)
        self.Q = np.array(Q[:, indices])
    ...
    def predict(self, X: torch.Tensor) -> np.ndarray:
        ...
        cluster_assignments = self._get_clusters_by_kmeans(embeddings)
    ...
    def transform(self, X):
        ...
        self.embeddings_ = self.embeddings_ @ self.Q
    ...
    def _build_laplacian(self, X):
        Dx = torch.cdist(X, X)
        Dis, indices = get_nearest_neighbors(X, k=self.spectral_n_nbg + 1)
        scale = compute_scale(
            Dis, k=self.spectral_scale_k, is_local=self.spectral_is_local_scale
        )
        W = get_gaussian_kernel(
            Dx, scale, indices, device=self.device, is_local=self.spectral_is_local_scale
        )
        L = get_laplacian(W)
        return L
